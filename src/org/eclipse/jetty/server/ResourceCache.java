...
public class ResourceCache implements HttpContent.Factory {
	...
	public class CachedHttpContent implements HttpContent {
		...
		public InputStream getInputStream() throws IOException {
			...
			if (indirect != null && indirect.hasArray()) {
				return new ByteArrayInputStream(indirect.array(), indirect.arrayOffset() + indirect.position(),
						indirect.remaining());
			}
        ...
        @Override
		public String toString() {
			return String.format("CachedContent@%x{r=%s,e=%b,lm=%s,ct=%s,gz=%b}", hashCode(), _resource,
					_resource.exists(), _lastModified, _contentType, _gzipped != null);
        }
        @Override
	...
	public class CachedGzipHttpContent extends GzipHttpContent {
		...
		CachedGzipHttpContent(CachedHttpContent content, CachedHttpContent contentGz) {
			...
			_etag = (ResourceCache.this._etags)
					? new PreEncodedHttpField(HttpHeader.ETAG, _content.getResource().getWeakETag("--gzip")) : null;
        }
