...
public class HttpConnection extends AbstractConnection implements Runnable, HttpTransport, Connection.UpgradeFrom {
	...
	private final HttpConfiguration _config;// HttpConfiguration
	private final Connector _connector;// ServerConnector
	private final ByteBufferPool _bufferPool;// ArrayByteBufferPool
    private final HttpInput _input;
	private final HttpGenerator _generator;// HttpGenerator
	private final HttpChannelOverHttp _channel;// HttpChannelOverHttp
	private final HttpParser _parser;// HttpParser
	...
	public HttpConnection(HttpConfiguration config, Connector connector, EndPoint endPoint, HttpCompliance compliance, boolean recordComplianceViolations) {
        ...
        _config = config;
		_connector = connector;// ServerConnector
		_bufferPool = _connector.getByteBufferPool();// ArrayByteBufferPool
        _generator = newHttpGenerator();
		_channel = newHttpChannel();// HttpChannelOverHttp
		_input = _channel.getRequest().getHttpInput();// HttpInputOverHTTP
		_parser = newHttpParser(compliance);// HttpParser
	...
	protected HttpChannelOverHttp newHttpChannel() {
		HttpChannelOverHttp httpChannel = new HttpChannelOverHttp(this, _connector, _config, getEndPoint(), this);// HTTP相关的数据创建都在此过程中
        return httpChannel;
    }
    ...
    @Override
	public void onFillable() {// 在哪调用???
    	
		System.out.println("HTTP生成过程: 1. HttpConnection");

    	System.out.println("请求处理过程: -1. HttpConnection");
		System.out.println("实现类: " + this.getClass().getName());// org.eclipse.jetty.server.HttpConnection
		...
		try {
			while (getEndPoint().isOpen()) {
				...
				System.out.println("HTTP生成过程: 2. HttpConnection");
				...
				if (handle) {
					...
					System.out.println("到此HTTP请求生成完成!下面进行处理过程");

					boolean suspended = !_channel.handle();// 处理请求
	...
	private int fillRequestBuffer() {
		...
		System.out.println("HTTP生成过程: 3. HttpConnection");
		...
		if (getCurrentConnection() != this) {
			...
			if (_parser.isStart()) {
				...
				if (BufferUtil.isEmpty(_requestBuffer)) {
                    // look for more data
                    fillInterested();
				} else if (getConnector().isRunning()) {// else if we are still
														// running
					try {// Dispatched to handle a pipelined request
			...
			} else if (getEndPoint().isOpen()) {// else the parser must be
												// closed,
												// so seek the EOF if we are
												// still open
				fillInterested();
			}
        }
	...
	public String toString() {
		return String.format("%s[p=%s,g=%s,c=%s]", super.toString(), _parser, _generator, _channel);
    }
	...
	private class SendCallback extends IteratingCallback {
		...
		private boolean reset(MetaData.Response info, boolean head, ByteBuffer content, boolean last,
				Callback callback) {
			if (reset()) {
                _info = info;
                _head = head;
		...
		public Action process() throws Exception {
			...
			while (true) {
                ...
                if (LOG.isDebugEnabled())
					LOG.debug("{} generate: {} ({},{},{})@{}", this, result, BufferUtil.toSummaryString(_header),
							BufferUtil.toSummaryString(_content), _lastContent, _generator.getState());
				...
				switch (result) {
					...
					case FLUSH: {
					// don't write the chunk or the content if this is a HEAD
					// response, or any other type of response that should have
					// no content
						if (_head || _generator.isNoContent()) {
                            BufferUtil.clear(chunk);
                            BufferUtil.clear(_content);
                        }
						if (BufferUtil.hasContent(_header)) {// if we have a header
