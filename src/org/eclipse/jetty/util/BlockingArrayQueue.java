...
public class BlockingArrayQueue<E> extends AbstractList<E> implements BlockingQueue<E> {
	...
	public static final int DEFAULT_CAPACITY = 128;// 默认CAPACITY
	...
	public boolean offer(E e) {
		...
		try {
			...
			if (size == _elements.length) {// should we expand array?
                _headLock.lock();
				try {
					if (!grow()) {
						return false;
			...
			int tail = _indexes[TAIL_OFFSET]; // re-read head and tail after a
												// possible grow
            _elements[tail] = e;
	...
	public void put(E o) throws InterruptedException {
		// the mechanism to await and signal when the queue is full is not
		// implemented
