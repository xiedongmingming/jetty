...
public class StringUtil {
     ...
     */
	public static int indexOfControlChars(String str) {// 获取字符串中包含的控制字符的位置
		if (str == null) {
            return -1;
        }
        int len = str.length();
    ...
    */
    ...
    public static String sanitizeXmlString(String html) {
		...
		int i = 0;
		loop: for (; i < html.length(); i++) {// are there any characters that need sanitizing?
		...
		if (i == html.length()) {// no characters need sanitizing, so return original string
			return html;
		}
		StringBuilder out = new StringBuilder(html.length() * 4 / 3);// create builder with OK content so far
		out.append(html, 0, i);
		for (; i < html.length(); i++) {// sanitize remaining content
			char c = html.charAt(i);
			switch (c) {
                case '&' :
                    out.append("&amp;");
