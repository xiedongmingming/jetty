            ...
            if (!HttpMethod.HEAD.is(request.getMethod()))
            {
                if (_etags)
                {
					...
					if (ifnm != null && etag != null) {
						...
						for (String tag : quoted) {
							if (etag.equals(tag)
									|| tag.endsWith(ETAG_GZIP_QUOTE) && etag.equals(removeGzipFromETag(tag))) {
                                response.setStatus(HttpServletResponse.SC_NOT_MODIFIED);
		...
		if (reqRanges == null || !reqRanges.hasMoreElements() || content_length < 0) {
			...
			for (int i = 0; i < ranges.size(); i++) {
				...
				length += ((i > 0) ? 2 : 0) + 2 + multi.getBoundary().length() + 2
						+ (mimetype == null ? 0 : HttpHeader.CONTENT_TYPE.asString().length() + 2 + mimetype.length())
						+ 2 + HttpHeader.CONTENT_RANGE.asString().length() + 2 + header[i].length() + 2 + 2
