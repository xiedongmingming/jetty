...
public class HttpGenerator {// 用于产生HTTP报文格式

    private final static Logger LOG = Log.getLogger(HttpGenerator.class);

	public final static boolean __STRICT = Boolean.getBoolean("org.eclipse.jetty.http.HttpGenerator.STRICT");// 系统属性
	...
	private final int _send;// 将要发送的值的和值
    ...
    public static void setJettyVersion(String serverVersion) {
        ...
        SEND[SEND_SERVER | SEND_XPOWEREDBY] = StringUtil.getBytes("Server: " + serverVersion + "\015\012X-Powered-By: " + serverVersion + "\015\012");
    }
	...
	public boolean isCommitted() {
		return _state.ordinal() >= State.COMMITTED.ordinal();// ???取出状态的顺序值
    }
	...
	private static final byte[][] SEND = new byte[][] { // 版本相关的初始值(版本号还未设置)
			new byte[0], 
    ...
    private static class PreparedResponse {// build cache of response lines for status
        byte[] _reason;
        byte[] _schemeCode;
        byte[] _responseLine;
    }
