...
public class GzipHttpContent implements HttpContent {
	...
	public static String removeGzipFromETag(String etag) {// 移除后面的标识
		if (etag == null) {
			return null;
		}
	...
	public String toString() {
        return String.format("GzipHttpContent@%x{r=%s|%s,lm=%s|%s,ct=%s}",
        		hashCode(),
                _content.getResource(),
                _contentGz.getResource(),
                _content.getResource().lastModified(),
                _contentGz.getResource().lastModified(),
                getContentType());
    }
	// *************************************************************************************************
}